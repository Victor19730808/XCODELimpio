//
//  GV_SCR_vg_EstablecimientosListaView_OLD.swift
//  limpioCS
//  Backup de la versión anterior de la lista de participantes
//
...
struct GV_SCR_vg_EstablecimientosListaView_OLD: View {
    ...
    @State private var distanceCache = DistanceCache_Old()
    ...
    private func obtenerDistanciaConCache(
        ...
        let estLocation: CLLocationCoordinate2D? = {
            guard let lat = establecimiento.direccion_latitud,
                  let lon = establecimiento.direccion_longitud else { return nil }
            return CLLocationCoordinate2D(latitude: lat, longitude: lon)
        }()
    ...
    private func calcularDistanciaHaversine(
        ...
        guard let userLoc = userLocation,
              let estLoc = estLocation else { return Double.infinity }
        let R: Double = 6371
        let lat1 = userLoc.latitude * .pi / 180
        let lon1 = userLoc.longitude * .pi / 180
        let lat2 = estLoc.latitude * .pi / 180
        let lon2 = estLoc.longitude * .pi / 180
        let dLat = lat2 - lat1
        let dLon = lon2 - lon1
...
#Preview {
    NavigationStack {
        GV_SCR_vg_EstablecimientosListaView_OLD()
